...
window.app = (function () {
  ...
  function Request() {
    (this._requestPjax = function (params) {
      ...
      jQuery(document).on(
        "pjax:success",
        function (event, data, status, jqXHR) {
      ...
      (this.post = function (params) {
        ...
        if (
          (!!window.safari === true || isIOSDevice) &&
          params.data instanceof FormData &&
          typeof params.data.getAll == "function"
        ) {
          var fileNameAttribute =
            params.data.getAll("filename[]").length > 0
              ? "filename[]"
              : "file[]";
  ...
  return {
    ...
    convertUrlToDataParams: function (url) {
      ...
      for (var index = 0; index < queryParameters.length; index++) {
        ...
        if (queryParamComponents[0] in params)
          params[queryParamComponents[0]] +=
            "&" + queryParamComponents[0] + "=" + queryParamComponents[1];
    ...
    getExtensionModuleName: function () {
      ...
      if (
        typeof _EXTENSIONMETA !== "undefined" &&
        _EXTENSIONMETA.hasOwnProperty("module")
      ) {
    ...
    getUserId: function () {
      if (_USERMETA) return _USERMETA.id;
    },
    getMenuStatus: function () {
      if (_USERMETA) return _USERMETA.menustatus;
    },
    ...
    convertCurrencyToUserFormat: function (value, appendCurrencySymbol) {
      ...
      if (groupingPattern == "123456789") {
        ...
        if (wholeFirstPart) {
          ...
          var wholeFirstParts = wholeFirstPart
            .toString()
            .split("")
            .reverse()
            .join("")
          ...
          for (var i = 0; i < wholeFirstParts.length; i++) {
            wholeFirstParts[i] = wholeFirstParts[i]
              .toString()
              .split("")
              .reverse()
              .join("");
