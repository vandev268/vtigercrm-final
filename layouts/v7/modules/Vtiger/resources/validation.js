...
jQuery.validator.addMethod(
  "Calendar_greaterThanDependentField",
  function (value, element, params) {
  ...
  jQuery.validator.format(
    app.vtranslate("JS_CHECK_START_AND_END_DATE_SHOULD_BE_GREATER")
  )
);

jQuery.validator.addMethod(
  "greaterThanDependentField",
  function (value, element, params) {
    var result = true;
    if (!value.length) {
      return result;
    ...
    if (
      typeof sourceFieldInfo == "undefined" ||
      typeof depFieldInfo == "undefined"
    )
      return result;

    if (
      (sourceFieldInfo.type !== "date" || depFieldInfo.type !== "date") &&
      (sourceFieldInfo.type !== "datetime" || depFieldInfo.type !== "datetime")
    ) {
      //start_date, end_date fields data type is datetime.
      // logging disabled
      return result;
    }
    ...
    if (closestForm.length > 0) {
      ...
      if (typeof depFieldVal === "undefined") {
        ...
        depFieldVal = jQuery(".fieldBasicData")
          .filter('[data-name="' + depFieldName + '"]')
          .data("displayvalue");
      }
    } else {
      var controller = app.controller();
    ...
    jQuery.validator.messages.greaterThanDependentField =
      sourceFieldInfo.label +
      " " +
...
jQuery.validator.addMethod(
  "greaterThanDependentRepeatTillDate",
  function (value, element, params) {
    var form = jQuery(element).closest("form");
    var recurringCheck = form
      .find('input[name="recurringcheck"]')
      .is(":checked");
  ...
  jQuery.validator.format(
    app.vtranslate("JS_REPEAT_DATE_SHOULD_BE_GREATER_THAN_START_DATE")
  )
);
...
jQuery.validator.addMethod(
  ...
  function (value, element, params) {
    ...
    for (var id in params) {
      ...
      if (
        recordId != id &&
        filterName.toLowerCase() == value.toLowerCase().trim()
      ) {
        duplicateExist = true;
        return false;
      }
...
jQuery.validator.addMethod(
  ...
  function (value, element, params) {
    try {
      if (value) {
        var fieldDateInstance = app.helper.getDateInstance(
          value,
          app.getDateFormat()
        );
...
jQuery.validator.addMethod(
  ...
  function (value, element, params) {
    ...
    if (closestForm.length > 0) {
      ...
      var controller = app.controller();
      var depFieldVal = jQuery(".fieldBasicData")
        .filter('[data-name="' + depFieldName + '"]')
        .data("displayvalue");
    ...
    jQuery.validator.messages.lessThanDependentField =
      sourceFieldInfo.label +
      " " +
      app.vtranslate("JS_SHOULD_BE_LESS_THAN_OR_EQUAL_TO") +
      " " +
...
jQuery.validator.addMethod(
    ...
    try {
      if (value == "Held") {
        ...
        for (var i = 0; i < params.length; i++) {
          ...
          var dependentFieldInContext = jQuery(
            "input[name=" + dependentField + "]",
            formElem
          );
          if (dependentFieldInContext.length > 0) {
            ...
            var dependentFieldDateInstance = app.helper.getDateInstance(
              fieldValue,
              dateFormat
            );
            var comparedDateVal =
              todayDateInstance - dependentFieldDateInstance;
            if (comparedDateVal < 0) {
              return false;
            }
          }
  ...
  jQuery.validator.format(
    "Status " + app.vtranslate("JS_FUTURE_EVENT_CANNOT_BE_HELD") + " Date Start"
  )
);
...
jQuery.validator.addMethod(
  ...
  jQuery.validator.format(
    app.vtranslate("JS_LENGTH_SHOULD_BE_LESS_THAN_EQUAL_TO") + " {0}"
  )
);
...
jQuery.validator.addMethod(
  ...
  jQuery.validator.format(
    app.vtranslate("JS_PLEASE_ENTER_NUMBER_IN_RANGE") +
      " {0} " +
      app.vtranslate("TO") +
      " {1}"
  )
...
jQuery.validator.addMethod(
  ...
  jQuery.validator.format(
    app.vtranslate("JS_VALUE_SHOULD_BE_GREATER_THAN_ZERO")
  )
);
...
jQuery.validator.addMethod(
  ...
  function (value, element, params) {
    var valid = jQuery.validator.methods.percentage.call(
      this,
      value,
      element,
      params
    ...
    if (valid) {
      jQuery.validator.messages.inventory_percentage = app.vtranslate(
        "JS_PERCENTAGE_SHOULD_BE_LESS_THAN_100"
      );
      return value > 100 ? false : true;
    } else {
      jQuery.validator.messages.inventory_percentage = app.vtranslate(
        "JS_ACCEPT_POSITIVE_NUMBER"
      );
      return valid;
    }
  },
  jQuery.validator.format(
    app.vtranslate("JS_PERCENTAGE_SHOULD_BE_LESS_THAN_100")
  )
);
...
jQuery.validator.addMethod(
  "reference_required",
  function (value, element, params) {
    var referenceValue = jQuery(element)
      .parent()
      .parent()
      .find("input.sourceField")
      .val();
    if (isNaN(referenceValue)) {
      referenceValue = jQuery(element)
        .parent()
        .parent()
        .find("input.sourceField")
        .data("value");
...
function calculateValidationRules(form, params, meta) {
  ...
  if (typeof meta === "undefined") {
    if (typeof uimeta === "undefined") {
      ...
      if (
        jQuery(".overlayEdit").length > 0 ||
        jQuery(".overlayDetail").length > 0
      ) {
        meta = related_uimeta;
      } else {
        meta = uimeta;
  ...
  for (var i = 0; i < inputElements.length; i++) {
    ...
    if (typeof fieldBasicInfo !== "undefined") {
      ...
      if (
        typeof fieldBasicInfo["validator"] !== "undefined" &&
        fieldBasicInfo["validator"].length > 0
      ) {
...
(function ($) {
  ...
  function getResolvedRules(container, meta) {
    ...
    elements.each(function (index, domElement) {
      ...
      for (var key in specificRulesMeta) {
        ...
        var moduleSpecificValidatorName =
          moduleName + "_" + specialValidatorName;
  ...
  $.fn.vtValidate = function (params) {
    ...
    $(document).on("select2-opening", function (arg) {
      ...
      if (
        select2Elem.find("ul").hasClass("input-error") ||
        select2Elem.find("a.select2-choice").hasClass("input-error")
      ) {
    ...
    this.on(
      "Vtiger.PostReference.Selection Vtiger.PostReference.QuickCreateSave",
      function (e) {
        var referenceWrapper = jQuery(e.target).closest(
          ".referencefield-wrapper"
        );
        var referenceElement = referenceWrapper.find(
