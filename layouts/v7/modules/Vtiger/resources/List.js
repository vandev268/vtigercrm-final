...
Vtiger.Class(
  "Vtiger_List_Js",
  {
      ...
      if (params) {
        ...
        app.request
          ...
          .then(function (error, data) {
            ...
            app.helper
              .loadPageContentOverlay(data, overlayParams)
    ...
    triggerTransferOwnership: function (massActionUrl) {
      ...
      if (listSelectParams) {
        app.helper.showProgress();
        app.request
          .get({ url: massActionUrl, data: listSelectParams })
          .then(function (error, data) {
            app.helper.hideProgress();
            if (data) {
    ...
    triggerMassAction: function (massActionUrl) {
      ...
      if (listSelectParams) {
        ...
        app.helper.showProgress();
        app.request.get({ data: postData }).then(function (err, data) {
          app.helper.hideProgress();
          if (data) {
  ...
  },
  {
    ...
    getRecordSelectTrackerInstance: function () {
      if (this.recordSelectTrackerInstance === false) {
        this.recordSelectTrackerInstance =
          Vtiger_RecordSelectTracker_Js.getInstance();
    ...
    resetData: function () {
      ...
      listViewContainer
        .find("#currentSearchParams")
        .val(JSON.stringify(new Array()));
      listViewContainer
        .find("#currentTagParams")
        .val(JSON.stringify(new Array()));
    ...
    getDefaultParams: function () {
      ...
      params.starFilterMode = container
        .find(".starFilter li.active a")
        .data("type");
    ...
    addStarSearchParams: function (listSearchParams) {
      var listViewContainer = this.getListViewContainer();
      var activeStarFilter = listViewContainer
        .find(".starFilter li.active")
        .find("a")
        .data("type");
    ...
    registerInlineEdit: function (currentTrElement) {
      ...
      for (var i = 0; i < tdElements.length; i++) {
        ...
        if (
          fieldName.match(/\((\w+)\s\;\s\((\w+)\)\s(\w+)\)/g) &&
          fieldName.match(/\((\w+)\s\;\s\((\w+)\)\s(\w+)\)/g).length
        ) {
          continue;
        }
        ...
        if (
          (typeof fieldBasicInfo == "undefined" ||
            fieldBasicInfo.length <= 0) &&
          typeof adv_search_uimeta !== "undefined"
        ) {
    ...
    validateAndSaveInlineEdit: function (currentTrElement) {
      ...
      var params = {
        ...
        submitHandler: function (form) {
          ...
          app.helper.showProgress();
          jQuery(".inline-save", currentTrElement)
            .find("button")
            .attr("disabled", "disabled");
    ...
    getInlineEditedFieldValue: function (fieldElement, inputElement) {
      ...
      if (jQuery.inArray(fieldType, picklistTypes) !== -1) {
        value = jQuery(".inputElement.select2", fieldElement)
          .find(":selected")
          .val();
    ...
    registerInlineSaveOnEnterEvent: function (editedRow) {
      editedRow.find(".inputElement:not(textarea)").on("keyup", function (e) {
        ...
        var fieldType = jQuery(e.target)
          .closest(".listViewEntryValue")
          .data("fieldType");
        if (ignoreList.indexOf(fieldType) !== -1) return;
        (e.keyCode || e.which) === 13 &&
          editedRow.find("button.save").trigger("click");
      });
    },
    /**
    ...
    registerListViewBasicActions: function () {
      ...
      app.event.on(
        "post.listViewInlineEdit.click",
        function (event, editedRow) {
    ...
    registerListViewSearch: function () {
      ...
      listViewPageDiv.on("click", '[data-trigger="listSearch"]', function (e) {
        ...
        listViewPageDiv
          .find('[data-trigger="clearListSearch"]')
          .removeClass("hide");
      ...
      listViewPageDiv.on(
        "click",
        '[data-trigger="clearListSearch"]',
        function (e) {
          e.preventDefault();
          listViewPageDiv
            .find('.listSearchContributor:not(".select2-container")')
            .each(function (i, contributor) {
      ...
      var listSearchContributorChangeHandler = function (e) {
        ...
        if (element.hasClass("select2")) {
          var currentElementContainer = element
            .closest(".select2_search_div")
            .find("div.listSearchContributor")
            .find("ul");
          var desireHeight = 150;
          if (currentElementContainer.height() > desireHeight) {
            currentElementContainer.css({
              cssText:
                "height:" + desireHeight + "px !important;" + "padding:0",
            });
          } else if (
            currentElementContainer.find(".mCSB_container").height() <
            desireHeight
          ) {
      ...
      listViewPageDiv
        .find(".searchRow div.listSearchContributor.select2")
        .each(function (i, elem) {
          var currentSearchInput = jQuery(elem);
          app.helper.showVerticalScroll(currentSearchInput.find("ul"), {
            height: 150,
          });
        });
      listViewPageDiv.on(
    ...
    registerAutoIncludeFieldsInMassEdit: function () {
      var autoIncludeFieldsInMassEditCallback = function () {
        ...
        $(this)
          .closest("tr")
          .find("input[id=include_in_mass_edit_" + fieldName + "]")
          .prop("checked", true);
      };
    ...
    markSelectedIdsCheckboxes: function () {
      ...
      if (selectAllMode == true) {
        jQuery("#deSelectAllMsgDiv")
          .closest("div.messageContainer")
          .addClass("show");
        ...
        if (!isEmpty) {
          ...
          if (
            listViewPageDiv.find(".listViewEntriesCheckBox").not(":checked")
              .length == 0
          ) {
            listViewPageDiv
              .find(".listViewEntriesMainCheckBox")
              .prop("checked", true);
          } else {
            listViewPageDiv
              .find(".listViewEntriesMainCheckBox")
    ...
    getListSelectAllParams: function (jsonDecode) {
      ...
      var params =
        recordSelectTrackerInstance.getSelectedAndExcludedIds(jsonDecode);
    ...
    getRecordsCount: function () {
      ...
      app.request.get(params).then(function (err, response) {
      ...
      app.request.post({ data: params }).then(function (err, data) {
        ...
        self.loadListViewRecords().then(function (e) {
          ...
          if (params.value == 1) {
            app.helper.showSuccessNotification({
              message:
                app.vtranslate("JS_FOLLOWING") +
                " " +
                selectedRecordCount +
                " " +
                app.vtranslate(params.module),
            });
          } else {
            app.helper.showSuccessNotification({
              message:
                app.vtranslate("JS_UNFOLLOWING") +
                " " +
                selectedRecordCount +
                " " +
                app.vtranslate(params.module),
    ...
    getStarRecordRows: function () {
      ...
      return listViewContainer
        .find("tr.listViewEntries .markStar.fa-star .active")
        .closest("tr");
    },
    getUnStarRecordRows: function () {
      var listViewContainer = this.getListViewContainer();
      return listViewContainer
        .find("tr.listViewEntries .markStar .fa-star-o")
        .closest("tr");
    },
    ...
    selectRecordsDependingOnStarFilter: function () {
      if (this.isStarFilterMode()) {
        this.getStarRecordRows()
          .find(".listViewEntriesCheckBox")
          .trigger("click");
      } else if (this.isUnStarFilterMode()) {
        this.getUnStarRecordRows()
          .find(".listViewEntriesCheckBox")
          .trigger("click");
      } else {
        jQuery("tr.listViewEntries")
          .find(".listViewEntriesCheckBox")
          .trigger("click");
      }
    },
    ...
    registerDeleteRecordClickEvent: function () {
      ...
      jQuery("#page").on("click", ".deleteRecordButton", function (e) {
        ...
        var originalDropDownMenu = elem
          .closest(".dropdown-menu")
          .data("original-menu");
        if (
          originalDropDownMenu &&
          typeof originalDropDownMenu != "undefined"
        ) {
    ...
    totalNumOfRecords: function (currentEle) {
      ...
      if (totalNumberOfRecords == 0) {
        ...
        thisInstance.getPageCount().then(function (data) {
          ...
          listViewContainer
            .find("ul#listViewPageJumpDropDown #totalPageCount")
            .text(data.page);
    ...
    registerMoreRecentUpdatesClickEvent: function (container, recordId) {
      container.find(".moreRecentUpdates").on("click", function () {
        ...
        var recentUpdateURL =
          ...
          encodeURIComponent(app.getModuleName()) +
          "&record=" +
    ...
    registerDynamicListHeaders: function () {
      ...
      jQuery(".viewColumnsList").on(
        ...
        function (e, params) {
          ...
          if ($(this).is(":checked")) {
            listHeaderFields = app.helper.array_merge(listHeaderFields, [
              $(this).val(),
            ]);
          } else {
    ...
    initializePaginationEvents: function () {
      ...
      listViewContainer.on("click", "#listViewReloadButton", function () {
        var urlParams = {};
        thisInstance.loadListViewRecords(urlParams);
        // Show a brief notification
        var params = {
          text: app.vtranslate("JS_LIST_REFRESHED"),
          type: "info",
          animation: "show",
        };
    ...
    addTagToList: function (tagElement) {
      ...
      if (
        moreTagTriggerEle.hasClass("hide") &&
        moreTagCount > 0 &&
        !container.find(".moreListTags").hasClass("hide")
      ) {
    ...
    addToExistingTagSelector: function (tagEle) {
      ...
      if (
        existingTagMenu.find('.tag[data-id="' + tagEle.data("id") + '"]')
          .length <= 0
      ) {
        var clonedTagEle = tagEle;
        clonedTagEle.find(".editTag").remove();
        var dummyTagListElement = existingTagMenu
          .find(".dummyExistingTagElement")
          .clone(true)
          .removeClass("dummyExistingTagElement");
        dummyTagListElement.find(".tag").replaceWith(clonedTagEle);
        dummyTagListElement
          .removeClass("hide")
          .appendTo(existingTagMenu.find("ul"));
      }
    },
    ...
    enableListViewActions: function () {
      jQuery(".btn-group.listViewActionsContainer")
        .find("button")
        .removeAttr("disabled");
      jQuery(".btn-group.listViewActionsContainer")
        .find("li")
    ...
    registerConfigureColumnsEvents: function () {
      ...
      listViewContentDiv.on("click", ".listColumnFilter", function (e) {
        ...
        var callback = function (container) {
          ...
          availFieldsListContainer.on(
            "click",
            '.availFieldBlock a[data-parent="#accordion"]',
            function (e) {
          ...
          selectedFieldsList.on("click", ".removeField", function (e) {
            ...
            if (selectedFieldsEles.length <= 1) {
              app.helper.showErrorNotification({
                message: app.vtranslate("JS_ATLEAST_SELECT_ONE_FIELD"),
              });
              return false;
            }
          ...
          availFieldsList.on("click", ".item", function (e) {
            ...
            var targetFieldEle = selectedFieldsListContainer
              .find(".item-dummy")
              .clone();
    ...
    registerEvents: function () {
      ...
      this.registerReloadTotalCountEvent();
    ...
    registerHeaderReflowOnListSearchSelections: function () {
      ...
      var thisInstance = this;
      var $table = jQuery("#listview-table");
      listViewContentDiv.on(
        "select2-selecting select2-close select2-removed",
        ".listSearchContributor",
        function () {
    ...
    registerDropdownPosition: function (container) {
      ...
      jQuery(".table-actions").on("click", ".dropdown", function (e) {
        ...
        if (container === "#page") {
          ...
          dropdownBottom =
            jQuery(window).height() - currtargetTop + dropdown.height();
        }
    ...
    reflowList: function () {
      if (
        typeof $.fn.perfectScrollbar !== "function" ||
        typeof $.fn.floatThead !== "function"
      ) {
        return;
      }
      var $table = jQuery("#listview-table");
