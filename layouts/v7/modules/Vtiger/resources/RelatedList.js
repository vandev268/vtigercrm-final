  ...
  },
  {
    ...
    getCompleteParams: function () {
      ...
      var searchParams = JSON.stringify(
        detailInstance.getRelatedListSearchParams()
      );
    ...
    loadRelatedList: function (params) {
      ...
      if (
        typeof this.relatedModulename == "undefined" ||
        this.relatedModulename.length <= 0
      ) {
        return;
      }
    ...
    addRelatedRecord: function (element, callback) {
      ...
      app.event.one("post.QuickCreateForm.show", function (event, data) {
        ...
        /*
            ...
            */
        jQuery(
          '<input type="hidden" name="sourceModule" value="' +
            parentModule +
            '" />'
        ).appendTo(data);
        jQuery(
          '<input type="hidden" name="sourceRecord" value="' + parentId + '" />'
        ).appendTo(data);
        jQuery(
      ...
      app.event.one("post.QuickCreateForm.save", function (event, data) {
        ...
        thisInstance.loadRelatedList().then(function (data) {
          ...
          if (thisInstance.relatedModulename == "Calendar") {
            ...
            thisInstance.updateRelatedRecordsCount(
              selectedTabElement.data("relation-id"),
              [1],
              true
            );
          }
      ...
      if (
        typeof fullFormUrl != "undefined" &&
        fullFormUrl.indexOf("?") !== -1
      ) {
        var urlSplit = fullFormUrl.split("?");
        var queryString = urlSplit[1];
        var queryParameters = queryString.split("&");
    ...
    registerScrollForRollupComments: function () {
      jQuery(document).scroll(function () {
        if (
          ...
          if (
            Vtiger_RelatedList_Js.loaded &&
            jQuery("#rollupcomments").attr("hascomments") == 1
          ) {
            ...
            var url =
              ...
              "&startindex=" +
              startindex +
              "&mode=getNextGroupOfRollupComments";
            ...
            app.request.get(params).then(function (err, data) {
              ...
              if (data) {
                ...
                jQuery(".commentsBody ul.unstyled:first").append(
                  jQuery(data).children()
                );
              } else {
    ...
    showPagingInfo: function () {
      var totalNumberOfRecords = jQuery(".relatedContainer")
        .find("#totalCount")
        .val();
    ...
    pageJumpOnSubmit: function (element) {
      ...
      var totalPages = parseInt(
        jQuery(".relatedContainer").find("#totalPageCount").text()
      );
      ...
      thisInstance.loadRelatedList(urlParams).then(function (data) {
    ...
    init: function (
      ...
      this.relatedTabsContainer = jQuery(selectedRelatedTabElement).closest(
        "div.related-tabs"
      );
      this.detailViewContainer = this.relatedTabsContainer.closest(
        "div.detailViewContainer"
      );
      this.relatedContentContainer = jQuery(
        "div.details",
...
jQuery(document).ready(function () {
  ...
  var instance = Vtiger_RelatedList_Js.getInstance(
    recordId,
    moduleName,
    selectedTabElement,
    relatedModuleName
