...
Vtiger.Class(
  "Settings_PickListDependency_Js",
  {
    ...
    triggerAdd: function (event) {
      ...
      instance.showEditView(instance.listViewForModule).then(function (data) {
    ...
    triggerEdit: function (event, module, sourceField, targetField) {
      ...
      instance.updatedSourceValues = [];
      instance
        .showEditView(module, sourceField, targetField)
        .then(function (data) {
          var form = jQuery("#pickListDependencyForm");
          form
            .find(
              '[name="sourceModule"],[name="sourceField"],[name="targetField"]'
            )
            .select2("disable");
      ...
      app.helper.showConfirmationBox({ message: message }).then(
        function (e) {
          instance
            ...
            .then(function (data) {
              var params = {};
              params.message = app.vtranslate(
                "JS_DEPENDENCY_DELETED_SUEESSFULLY"
              );
  ...
  },
  {
    ...
    getModuleDependencyGraph: function (form) {
      ...
      form.find('[name="sourceModule"]').on("change", function () {
        ...
        thisInstance.showEditView(forModule).then(function (data) {
          thisInstance.registerAddViewEvents();
          thisInstance.registerTargetFieldsClickEvent(
            jQuery("#dependencyGraph")
          );
        });
      });
    },
    ...
    registerPicklistFieldsChangeEvent: function (form) {
      var thisInstance = this;
      form
        .find('[name="sourceField"],[name="targetField"]')
        .on("change", function () {
    ...
    checkValuesForDependencyGraph: function (form) {
      ...
      if (sourceFieldValue != "" && targetFieldValue != "") {
        var result = app.vtranslate(
          "JS_SOURCE_AND_TARGET_FIELDS_SHOULD_NOT_BE_SAME"
        );
     ...
     */
    checkCyclicDependency: function (
      sourceModule,
      sourceFieldValue,
      targetFieldValue
    ) {
        ...
        element
          ...
          .each(function (i, row) {
            if (jQuery(row).css("display") != "none") {
              ...
              if (
                jQuery.inArray(sourceValue, thisInstance.updatedSourceValues) ==
                -1
              ) {
    ...
    registerListViewEvents: function () {
      var thisInstance = this;
      var forModule = jQuery(".settingsPageDiv div")
        .find(".pickListSupportedModules")
        .val();
    ...
    registerTargetFieldsClickEvent: function (dependencyGraph) {
      ...
      dependencyGraph.on("click", "td.picklistValueMapping", function (e) {
        ...
        if (
          jQuery.inArray(sourceValue, thisInstance.updatedSourceValues) == -1
        ) {
          thisInstance.updatedSourceValues.push(sourceValue);
        }
        if (currentTarget.hasClass("selectedCell")) {
          currentTarget
            .addClass("unselectedCell")
    ...
    registerSelectSourceValuesClick: function (dependencyGraph) {
      ...
      dependencyGraph.find("button.sourceValues").click(function () {
        ...
        var clonedContainer = selectSourceValues
          .clone(true, true)
          .removeClass("modalCloneCopy");
        var callBackFunction = function (data) {
          ...
          data.find('[name="saveButton"]').click(function (e) {
            ...
            jQuery.each(sourceValues, function (index, ele) {
              ...
              var hiddenElement = selectSourceValues.find(
                '[class*="' + encodedValue + '"]'
              );
    ...
    registerSelectAllSourceValuesClick: function (dependencyGraph) {
      ...
      dependencyGraph.find("button.selectAllValues").on("click", function () {
        var selectedElements = dependencyGraph
          .find(".dependencyMapping td.picklistValueMapping")
          .filter(":visible")
          .filter(".unselectedCell");
    ...
    loadMappingForSelectedValues: function (dependencyGraph) {
      var thisInstance = this;
      var allSourcePickListValues = jQuery.parseJSON(
        dependencyGraph.find(".allSourceValues").val()
      );
      var dependencyTable = dependencyGraph.find(".pickListDependencyTable");
      ...
      app.request.post({ data: data }).then(function (error, data) {
        ...
        if (!error) {
          app.helper.showSuccessNotification({
            message: app.vtranslate("JS_PICKLIST_DEPENDENCY_SAVED"),
          });
          thisInstance.loadListViewContents(thisInstance.listViewForModule);
        }
      });
    ...
    loadListViewContents: function (forModule) {
      ...
      app.request.pjax({ data: params }).then(function (error, data) {
        ...
        vtUtils.showSelect2ElementView(
          jQuery(".settingsPageDiv div").find(".pickListSupportedModules")
        );
