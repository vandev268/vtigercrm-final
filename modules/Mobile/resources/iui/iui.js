...
(function () {
    ...
    showPage: function (page, backwards) {
      if (page) {
        ...
        if (hasClass(page, "dialog")) showDialog(page);
          ...
          if (fromPage) {
            if (fromPage.onblur) fromPage.onblur();
          ...
          if (lNewPage) {
            ...
            if (page.getAttribute("script"))
              loadScript(page.getAttribute("script"), function () {
                continueLoadingPage(page, lNewPage);
              });
            else continueLoadingPage(page, lNewPage);
          } else continueLoadingPage(page, lNewPage);
        }
      }
    },
  ...
  addEventListener(
    ...
    function (event) {
      if (
        hasClass(findParent(event.target, "form"), "dialog") ||
        hasClass(findParent(event.target, "form"), "panel")
      ) {
  ...
  addEventListener(
    ...
    function (event) {
      ...
      if (link) {
        ...
        } else if (link == $("backButton")) history.back();
  ...
  function showDialog(page) {
    ...
    if (hasClass(page, "dialog") && !page.target) showForm(page);
  }
  ...
  function submitForm(form) {
    // CWZ
    if (form.getAttribute("target") == "_self") form.submit();
    else
      ...
      for (var i = 0; i < inputs.length; ++i) {
        if (inputs[i].name) {
          if (
            ...
            continue;
          } else args.push(inputs[i].name + "=" + escape(inputs[i].value));
        }
      }
    }
  ...
  function findParent(node, localName) {
    while (
      node &&
      (node.nodeType != 1 || node.localName.toLowerCase() != localName)
    )
    ...
    try {
      ...
      if (!actionButtons[page.id]) {
        ...
        if (page.getAttribute("actionbutton")) {
          ...
          // stores the fact that the page has no button
          lSet = true;
      ...
      if (!lSet) {
        btn.style.visibility = actionButtons[page.id].visible
          ? "visible"
          : "hidden";

        if (actionButtons[page.id].title != null)
          btn.innerHTML = actionButtons[page.id].title;
        if (actionButtons[page.id].href != null)
          btn.href = actionButtons[page.id].href;
        if (actionButtons[page.id].target != null)
          btn.target = actionButtons[page.id].target;
      }
    } catch (ex) {
