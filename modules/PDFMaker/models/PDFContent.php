...
class PDFMaker_PDFContent_Model extends PDFMaker_PDFContentUtils_Model {
    ...
    private function getInventoryProducts($module, $focus){
        if (!empty($focus->id)) {
            ...
            foreach ($relatedProducts AS $i => $PData) {
                ...
                if (PDFMaker_Utils_Helper::count($PData["subprod_qty_list".$sequence]) > 0) {
                ...
                if ($taxtype == "individual"){
                    ...
                    for ($tax_count = 0; $tax_count < PDFMaker_Utils_Helper::count($tax_details); $tax_count++){
                    ...
                    if (PDFMaker_Utils_Helper::count($Tax_Values) > 0) {
	    ...
	    if ($taxtype !== "individual") {
	        if (PDFMaker_Utils_Helper::count($finalDetails["taxes"]) > 0){
    ...
    public function retrieveAssignedUserId()
    {
        if (empty(self::$focus->column_fields['assigned_user_id'])) {
            $userIdResult = self::$db->pquery("SELECT smownerid FROM vtiger_crmentity WHERE crmid = ?", array(self::$focus->id));
            self::$focus->column_fields["assigned_user_id"] = self::$db->query_result($userIdResult, 0, "smownerid");
        }
    }
            ...
            while ($row = self::$db->fetch_array($result)){
                ...
                if ($related_module != ""){
                    ...
                    if ($fk_record != "" && $fk_record != "0"){
                        if ($related_module == "Users"){ $control_sql = "vtiger_users WHERE id="; } else { $control_sql = "vtiger_crmentity WHERE crmid="; }    
            ...
            foreach ($html->find("rowbreak") as $pb){
                if ($pb->outertext == self::$rowbreak){
                    ...
                    if ($tmpPb->tag == "td"){
                        ...
                        for ($i = 0; $i < (PDFMaker_Utils_Helper::count($partsArr) - 1); $i++){
                            $tmpPb->innertext = $partsArr[$i];
    
                            $addition = '<tr>';
                            for ($j = 0; $j < PDFMaker_Utils_Helper::count($prev_sibling_styles); $j++){
                                $addition.='<td ' . $prev_sibling_styles[$j] . '>&nbsp;</td>';
                            }
                            $addition.='<td style="' . $tmpPb->getAttribute("style") . '">' . $partsArr[$i + 1] . '</td>';
                            for ($j = 0; $j < PDFMaker_Utils_Helper::count($next_sibling_styles); $j++){
                        ...
                        $toSkip = PDFMaker_Utils_Helper::count($partsArr) - 2;
                    }
                }
            }
            self::$content = $html->save();
        ...
        foreach ($stucturedValues AS $BLOCK_LABEL => $BLOCK_FIELDS) {
            ...
            foreach ($BLOCK_FIELDS AS $FIELD_NAME => $FIELD_MODEL) {
                ...
                if ($inventory_currency !== false){
                    ...
                    //self::$rep["$" . $related . strtoupper($convEntity . "_" . $fieldname) . "/value$"] = $fieldvalue;
                    //self::$rep["$" . $related . strtoupper($convEntity . "_" . $fieldname) . "/type$"] = $fieldDataType;
                }
            }
        }
    ...
    private function replaceInventoryDetailsBlock($module, $focus, $is_related = false){
        ...
        if ($is_related === false) {
            $vattable = "";
            if (PDFMaker_Utils_Helper::count($Products["TOTAL"]["VATBLOCK"]) > 0) {
